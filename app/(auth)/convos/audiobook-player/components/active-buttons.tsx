...
function ActiveButton({ limit, title, isReaderView }: { limit: number; title: string; isReaderView?: boolean }) {
  ...
  const setActiveTimeLimit = usePlayerViewModeStore((state) => state.setActiveTimeLimit);
  const activeLengthLimit = usePlayerViewModeStore((state) => state.activeLengthLimit);
  const setActiveLengthLimit = usePlayerViewModeStore((state) => state.setActiveLengthLimit);

  const activeValue = isReaderView ? activeLengthLimit : activeTimeLimit;
  const setActiveValue = isReaderView ? setActiveLengthLimit : setActiveTimeLimit;
...
export const ActiveButtons = ({ isReaderView }: { isReaderView?: boolean }) => {
  ...
  return (
    <div className="space-x-4 sm:text-xl flex justify-start">
      {options.map((option) => {
        return <ActiveButton key={JSON.stringify(option)} {...option} isReaderView={isReaderView} />;
      })}
    </div>
  );
};
