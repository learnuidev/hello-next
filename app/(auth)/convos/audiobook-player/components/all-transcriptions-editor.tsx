...
import { RawTranscriptEditor } from "./raw-transcript-editor";
...
export const AllTranscriptionsEditor = ({
  ...
  const [rawHasErrors, setRawHasErrors] = React.useState(false);
  // Bumped when the cues are replaced from elsewhere (subtitle upload), so the
  // raw editor rebuilds its blocks from the new cues.
  const [rawSourceVersion, setRawSourceVersion] = React.useState(0);
  ...
  const handleUploadSubtitles = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    fileReader.onload = (event) => {
      ...
      try {
        ...
        // Rebuild the raw blocks from the uploaded cues when it is open.
        setRawSourceVersion((version) => version + 1);
      } catch (error) {
    ...
    <div className="flex flex-col  mb-80 h-[75vh] overflow-hidden">
      <div className="flex gap-4 mb-8 sticky top-0 bg-white dark:bg-[rgb(9,10,11)] z-10 py-4 px-4 border-b border-gray-100 dark:border-[rgb(25,26,30)]">
        ...
        <button
          ...
          onClick={handleSave}
          disabled={updateContentMutation.isPending || rawHasErrors}
        >
      ...
      <div className="flex gap-6 h-full px-4">
        <div className="w-[70%] overflow-y-auto pr-4">
          {editorMode === "raw" ? (
            <RawTranscriptEditor
              key={rawSourceVersion}
              transcriptions={transcriptions}
              onChange={setLocalTranscriptions}
              onErrorsChange={setRawHasErrors}
            />
          ) : (
            <>
