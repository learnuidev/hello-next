...
type CountdownOptions = {
  ...
  /**
   * Callback function that runs when the counter ends
   * @default 0
   */
  onCountdownEnd?: () => void;
};
...
type CountdownContdownResponse = {
  /** Count down value. */
  count: number;
  /** Is the countown value running. */
 ...
 * @returns {[number, CountdownContdownResponse]} An array containing the countdown's count and its controllers.
 * @public
...
export function useCountdown({
  ...
  isIncrement = false,
  onCountdownEnd,
}: CountdownOptions): CountdownContdownResponse {
  const {
    count,
    increment,
    decrement,
